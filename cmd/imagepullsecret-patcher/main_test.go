...
func helperCreateValidSecret(k8s *k8sClient) error {
	...
	_, err := k8s.clientset.CoreV1().Secrets(corev1.NamespaceDefault).Create(ctx, dockerconfigSecret(corev1.NamespaceDefault), metav1.CreateOptions{})
	return err
}
...
func helperCreateServiceAccountWithImagePullSecret(secretName, serviceAccountName string) step {
	return func(k8s *k8sClient) error {
		ctx := context.Background()
		_, err := k8s.clientset.CoreV1().ServiceAccounts(corev1.NamespaceDefault).Create(ctx,
			&corev1.ServiceAccount{
				ObjectMeta: metav1.ObjectMeta{
					Name:      serviceAccountName,
					Namespace: corev1.NamespaceDefault,
...
func assertNoSecret(k8s *k8sClient) error {
	ctx := context.Background()
	_, err := k8s.clientset.CoreV1().Secrets(corev1.NamespaceDefault).Get(ctx, configSecretName, metav1.GetOptions{})
...
func assertSecretIsValid(k8s *k8sClient) error {
	ctx := context.Background()
	secret, err := k8s.clientset.CoreV1().Secrets(corev1.NamespaceDefault).Get(ctx, configSecretName, metav1.GetOptions{})
	if err != nil {
...
func assertHasImagePullSecret(secretName, serviceAccountName string) step {
	return func(k8s *k8sClient) error {
		ctx := context.Background()
		sa, err := k8s.clientset.CoreV1().ServiceAccounts(corev1.NamespaceDefault).Get(ctx, serviceAccountName, metav1.GetOptions{})
		if err != nil {
			return err
		}
